...
// RNGH's own (non-exported) GestureRef type models an external-component ref
// as RefObject<React.ComponentType> — i.e. the component's definition — but a
// ScrollView/FlatList ref (as returned by useRef<ScrollView>(null) etc.) is a
// ref to the mounted INSTANCE, which is what every real caller (and RNGH's own
// runtime resolution, via findNodeHandle) actually needs. This cast bridges
// that overly-narrow declaration; the value itself is unchanged, and this is
// the only place the mismatch is worked around.
function asGestureRef(ref: RefObject<unknown>): any {
  return ref;
}
...
function DragHandle({ index, total, translationY, handleRef, columnName, canMovePrevColumn, canMoveNextColumn, onBegin, onMove, onEnd, onCancel, onReorder, onMoveColumn, scrollRef, pagerRef }: { index: number; total: number; translationY: SharedValue<number>; handleRef: ReturnType<typeof useAnimatedRef<View>>; columnName: string; canMovePrevColumn: boolean; canMoveNextColumn: boolean; onBegin: () => void; onMove: (dx: number, dy: number, pageX: number, pageY: number) => void; onEnd: () => void; onCancel: () => void; onReorder: (direction: -1 | 1) => void; onMoveColumn: (direction: -1 | 1) => void; scrollRef: RefObject<ScrollView | null>; pagerRef: RefObject<FlatList<CarouselItem> | null> }) {
  ...
  //
  // blocksExternalGesture(scrollRef, pagerRef): on iOS, a plain (non-RNGH)
  // ScrollView/FlatList's native UIScrollView pan recognizer and this RNGH pan
  // candidate are siblings with no relationship by default — nothing stops the
  // scroll view from recognizing and stealing the touch while this candidate is
  // still waiting out its long-press timer (Android's touch dispatch is far more
  // forgiving here, which is why this only surfaced on iOS). This tells iOS the
  // scroll/pager must wait for this candidate to fail (ordinary scroll/swipe) or
  // succeed (long press) before it's allowed to start.
  const gesture = useMemo(() => Gesture.Pan().activateAfterLongPress(200).failOffsetX([-10, 10]).failOffsetY([-10, 10]).blocksExternalGesture(asGestureRef(scrollRef), asGestureRef(pagerRef)).onStart(() => {
    translationY.set(0);
    runOnJS(onBegin)();
  }).onUpdate((event) => {
    translationY.set(event.translationY);
    runOnJS(onMove)(event.translationX, event.translationY, event.absoluteX, event.absoluteY);
  }).onEnd(() => { runOnJS(onEnd)(); }).onFinalize((_, success) => { if (!success) runOnJS(onCancel)(); }), [onBegin, onCancel, onEnd, onMove, pagerRef, scrollRef, translationY]);
...
// Default React.memo comparison, reimplemented — needed because CardSurface's own
// comparator (below) must fall through to the *same* check it would otherwise get
// for free, for every prop except `frozen`.
...
const CardSurface = memo(function CardSurface({ card, index, total, translationY, dragging = false, interactive = true, highlighted = false, accent, accentTint, accentBorderColor, columnName = '', canMovePrevColumn = false, canMoveNextColumn = false, onBeginDrag, onDragMove, onDragEnd, onDragCancel, onOpenMove, onReorder, onMoveColumn, scrollRef, pagerRef, activeDragCardId }: { card: Card; index: number; total: number; translationY?: SharedValue<number>; dragging?: boolean; interactive?: boolean; highlighted?: boolean; accent?: string; accentTint?: string; accentBorderColor?: string; columnName?: string; canMovePrevColumn?: boolean; canMoveNextColumn?: boolean; onBeginDrag?: () => void; onDragMove?: (dx: number, dy: number, pageX: number, pageY: number) => void; onDragEnd?: () => void; onDragCancel?: () => void; onOpenMove?: () => void; onReorder?: (direction: -1 | 1) => void; onMoveColumn?: (direction: -1 | 1) => void; scrollRef: RefObject<ScrollView | null>; pagerRef: RefObject<FlatList<CarouselItem> | null>; frozen?: boolean; activeDragCardId?: SharedValue<string | null> }) {
  ...
  // compiler's own memoization pass.
  'use no memo';
  const { tokens: theme } = useTheme();
  ...
  // See the blocksExternalGesture note below — a ref to the card's own outer
  // Pressable, so the drag gesture can also block ITS tap recognition.
  ...
  // Hides this exact card in place once it becomes the live-dragged one — read
  // reactively off activeDragCardId on the UI thread, not a React prop, so it
  // never triggers (and can't be caught up in) a re-render of this component
  // while frozen. See the freeze comment on the memo comparator below.
  // Always returns the same set of keys (position/opacity/zIndex), just toggling
  // their values — returning a DIFFERENT key set per call (e.g. {} when visible)
  // is a known Reanimated footgun: a key that disappears from one call to the next
  // isn't reliably reset back to its stylesheet default on the native side, so
  // opacity: 0 was getting stuck permanently the instant a drag ended, leaving the
  // card invisible.
  const dragHiddenStyle = useAnimatedStyle(() => {
  ...
  //
  // The guard below checks each callback individually (not a single combined
  // `draggable` flag) so TS narrows every one to defined for the rest of this
  // closure — a bundled boolean can't carry that per-variable information, and
  // an uncoerced `&&` chain of them would evaluate to whichever callback happens
  // to sit last (onReorder), whose identity isn't stable across renders (it's
  // rebuilt whenever a card's `index` prop shifts during a drag) — silently
  // smuggling that unstable reference into this useMemo's dependencies and
  // recreating (and RNGH re-attaching) the live Pan gesture on every reorder,
  // cancelling the in-flight drag.
  const cardGesture = useMemo(() => {
    if (!interactive || !translationY || !onBeginDrag || !onDragMove || !onDragEnd || !onDragCancel || !onReorder) return undefined;
    ...
    // blocksExternalGesture: see the matching note on DragHandle's gesture above
    // for scrollRef/pagerRef — without it, iOS lets the vertical column
    // ScrollView (and, more rarely, the horizontal column pager) recognize and
    // steal the touch while this candidate is still waiting out its long-press
    // timer, so the drag never activates. cardRef is the same fix applied to
    // the card's OWN outer Pressable: a plain RN Pressable installs its own
    // separate native touch responder on the very same view this Pan gesture is
    // attached to (via the GestureDetector wrapping it below), and without an
    // explicit relation, iOS can let that responder's tap recognition win the
    // touch before the long-press timer completes, so the drag is silently
    // swallowed by "open card details" instead. Touches that land on the
    // handle/footer already fail this candidate instantly (below), so blocking
    // the Pressable there resolves again almost immediately — those buttons
    // stay responsive.
    // asGestureRef never reads `.current` itself — it only relabels the ref's
    // type so RNGH's relation resolver (which reads it natively, on demand,
    // not during this render) will accept it. The react-hooks/refs rule can't
    // see that from a generic function call.
    // eslint-disable-next-line react-hooks/refs
    return Gesture.Pan().activateAfterLongPress(320).failOffsetX([-10, 10]).failOffsetY([-10, 10]).blocksExternalGesture(asGestureRef(scrollRef), asGestureRef(pagerRef), asGestureRef(cardRef))
  ...
  const cardSurface = <Pressable ref={cardRef} disabled={!interactive} accessibilityRole="button" accessibilityLabel={`Open ${card.title || card.preview || 'untitled thought'}${accessibleMetadata ? `, ${accessibleMetadata}` : ''}`} accessibilityHint="Opens card details" onPress={() => router.push(`/card/${card.id}`)} style={({ pressed }) => [styles.card, { backgroundColor: theme.surface, borderColor: theme.borderSubtle }, dragging && styles.cardDragging, pressed && interactive && styles.cardPressed]}>
    ...
    <View style={styles.cardBody}>
      ...
      <View ref={footerRef} collapsable={false} style={styles.cardControls}>
        <Pressable disabled={!interactive} hitSlop={{ top: 6, bottom: 6 }} accessibilityRole="button" accessibilityLabel="Move to another column" onPress={(event) => { event.stopPropagation(); onOpenMove?.(); }} style={({ pressed }) => [styles.move, { backgroundColor: theme.surfaceElevated }, !interactive && styles.reorderButtonDisabled, pressed && styles.controlPressed]}><Ionicons accessible={false} name="swap-horizontal-outline" size={15} color={resolvedAccent} /><Text style={[styles.moveText, { color: resolvedAccent }]}>Move</Text></Pressable>
        <View style={[styles.reorderGroup, { backgroundColor: theme.surfaceElevated }]}>
          <Pressable disabled={!interactive || index === 0} hitSlop={{ top: 6, bottom: 6 }} accessibilityRole="button" accessibilityLabel="Move card up" accessibilityState={{ disabled: !interactive || index === 0 }} onPress={(event) => { event.stopPropagation(); onReorder?.(-1); }} style={({ pressed }) => [styles.reorderButton, (index === 0 || !interactive) && styles.reorderButtonDisabled, pressed && styles.reorderButtonPressed]}><Ionicons accessible={false} name="arrow-up-outline" size={15} color={resolvedAccent} /></Pressable>
          <View style={[styles.reorderDivider, { backgroundColor: theme.borderSubtle }]} />
          <Pressable disabled={!interactive || index === total - 1} hitSlop={{ top: 6, bottom: 6 }} accessibilityRole="button" accessibilityLabel="Move card down" accessibilityState={{ disabled: !interactive || index === total - 1 }} onPress={(event) => { event.stopPropagation(); onReorder?.(1); }} style={({ pressed }) => [styles.reorderButton, (index === total - 1 || !interactive) && styles.reorderButtonDisabled, pressed && styles.reorderButtonPressed]}><Ionicons accessible={false} name="arrow-down-outline" size={15} color={resolvedAccent} /></Pressable>
        </View>
      </View>
    </View>
  </Pressable>;
  return <Animated.View style={dragHiddenStyle}>{cardGesture ? <GestureDetector gesture={cardGesture}>{cardSurface}</GestureDetector> : cardSurface}</Animated.View>;
}, (prev, next) => {
  // While this card owns the live drag gesture, its render is frozen outright —
  // proven via instrumentation: re-rendering it during an active touch (for ANY
  // reason — a reorder-driven index shift, a hidden-state style toggle, even with
  // every other prop held byte-for-byte identical) reliably tears down and
  // recreates its native Pan recognizer mid-gesture on this RN/Reanimated/RNGH
  // combination, cancelling the drag. `frozen` is the one prop allowed to bypass
  // the freeze, so rendering resumes normally the instant the drag ends.
...
export default function BoardScreen() {
  ...
  // Which card is currently the live-dragged one, read by that card's own
  // CardSurface via useAnimatedStyle to hide itself in place — a UI-thread-only
  // reactive read, not a React prop, so setting it never re-renders (and can't
  // tear down) the component whose native gesture is in the middle of that exact
  // touch. See the freeze comment on CardSurface's memo comparator for why that
  // matters here specifically.
  const activeDragCardId = useSharedValue<string | null>(null);
  ...
  const beginDrag = useCallback((card: Card, index: number) => { const column = currentColumnRef.current; const layout = layoutsRef.current[card.id]; if (!layout || dragRef.current || !column) return; const next = { card, sourceColumnId: column.id, destinationColumnId: column.id, fromIndex: index, toIndex: index, height: layout.height, startY: layout.y, overlayTop: 76 + layout.y - scrollYRef.current, startScrollY: scrollYRef.current }; activeDragCardId.set(card.id); gestureTranslationX.set(0); gestureTranslationY.set(0); scrollCompensation.set(0);
    // ~150ms settle, minimal overshoot — communicates "picked up," not "bounced."
    liftProgress.set(reduceMotion ? 0 : withSpring(1, { damping: 26, stiffness: 380, mass: 0.6 })); dragRef.current = next; setDrag(next); setError(null); void Haptics.impactAsync(Haptics.ImpactFeedbackStyle.Light); }, [activeDragCardId, gestureTranslationX, gestureTranslationY, liftProgress, reduceMotion, scrollCompensation]);
  ...
  const clearDrag = useCallback(() => { dragRef.current = null; setDrag(null); lastDragPageYRef.current = 0; clearEdgeDwell(); activeDragCardId.set(null); gestureTranslationX.set(0); gestureTranslationY.set(0); scrollCompensation.set(0); liftProgress.set(0); }, [activeDragCardId, clearEdgeDwell, gestureTranslationX, gestureTranslationY, liftProgress, scrollCompensation]);
  const finalizeDrag = useCallback(async (cancelled: boolean, targetIndex: number) => { const active = dragRef.current; if (!active) { clearDrag(); return; } if (cancelled || (active.destinationColumnId === active.sourceColumnId && targetIndex === active.fromIndex)) { clearDrag(); return; } const previousCards = cards; if (active.destinationColumnId !== active.sourceColumnId) {
      ...
      // The real card list is only committed (and the overlay/hidden-source-card
      // cleared) once the overlay's settle spring below actually finishes — never
      // before. Committing earlier reflows both columns via LinearTransition while the
      // overlay is still independently animating on top of that new layout, which is
      // what produced the flicker/"snap then slide" on cross-column drops.
      const commit = () => {
      ...
      requestAnimationFrame(() => {
        // Same formula as the same-column settle below: spring toward the destination
        // placeholder's actual measured slot, not a fixed screen offset — the previous
        // `76 - overlayTop` always sprang to the top of the column regardless of drop
        // index, which is what produced the "wrong position" flash mid-list.
        const destinationY = (placeholderYRef.current || active.startY) - active.startY - scrollCompensation.get();
        liftProgress.set(withSpring(0, { damping: 24, stiffness: 300 }));
        if (reduceMotion) { gestureTranslationX.set(0); gestureTranslationY.set(destinationY); commit(); return; }
        gestureTranslationX.set(withSpring(0, { damping: 24, stiffness: 300 }));
        gestureTranslationY.set(withSpring(destinationY, { damping: 24, stiffness: 300, mass: 0.72 }, (finished) => { if (finished) runOnJS(commit)(); }));
      });
      return;
    }
    const sourceCards = cards.filter((card) => card.columnId === active.sourceColumnId).sort((a, b) => a.position - b.position);
    const ordered = reorder(sourceCards, active.fromIndex, targetIndex); setCards((all) => [...all.filter((card) => card.columnId !== active.sourceColumnId), ...ordered]); clearDrag(); try { await repository.moveCard(active.card.id, active.sourceColumnId, targetIndex); await Haptics.notificationAsync(Haptics.NotificationFeedbackType.Success); } catch { setCards(previousCards); setError('Chits could not save that card position. Try again.'); } }, [cards, clearDrag, columnInterval, columns, gestureTranslationX, gestureTranslationY, liftProgress, reduceMotion, repository, scrollCompensation]);
  ...
  const buildLaneRows = (column: Column, laneCards: Card[]): ReactNode[] => {
    ...
    // The dragged card's own wrapper below is rendered with the EXACT SAME props
    // (key, layout, entering, onLayout, no style) as every other card's wrapper in
    // this loop, and its own index (drag.fromIndex) never changes for the rest of
    // the gesture — proven via instrumentation that even a single differing prop
    // (a style change, a moved array position, anything) reliably got this element
    // torn down and recreated mid-gesture on this RN/Reanimated/RNGH combination,
    // destroying the native recognizer mid-touch. `frozen` (CardSurface's memo
    // comparator) and hiding it via activeDragCardId (a shared value CardSurface
    // reads reactively, not a prop) are what actually keep it out of the reach of
    // this render entirely — see both of those for the full explanation.
    for (const card of laneCards) {
      const isDragged = isSource && card.id === drag.card.id;
  ...
  return <Screen edges={['top', 'left', 'right']}><View style={styles.header}>
    ...
    <View style={styles.pager} accessibilityLabel={`${board.name} board. Column ${columnIndex + 1} of ${columns.length}. ${activeCardCount} ${activeCardCount === 1 ? 'card' : 'cards'}.`}>{columns.length ? <View style={styles.carousel} onLayout={(event) => { const height = event.nativeEvent.layout.height; event.currentTarget.measureInWindow((_, y) => { viewportRef.current = { top: y, height }; }); }}>
      ...
      <Pressable accessibilityRole="button" accessibilityLabel="Add from Unorganized" accessibilityHint="Opens unorganized thoughts to add to this board" onPress={() => router.push(`/unorganized?boardId=${id}`)} style={({ pressed }) => [styles.add, { bottom: addButtonBottom, backgroundColor: accent }, pressed && styles.addPressed]}><Ionicons accessible={false} name="add-outline" size={28} color={accentOn} /></Pressable>
...
const styles = StyleSheet.create({
  ...
  activeLane: { flex: 1 }, cards: { flexGrow: 1, paddingHorizontal: spacing.md, paddingBottom: spacing.lg }, cardList: { position: 'relative', gap: spacing.xs },
  ...
  skeletonStack: { gap: spacing.xs, paddingHorizontal: spacing.md, paddingTop: spacing.xs }, skeletonBar: { borderRadius: 6 }, skeletonTitleBar: { width: '68%', height: 16 }, skeletonPreviewBar: { width: '92%', height: 13, marginTop: spacing.xxs }, skeletonHandle: { width: 44, height: 44, borderRadius: 12, marginTop: -spacing.xs, marginRight: -spacing.xs }, skeletonFooterBar: { height: 32, borderRadius: 10 },
  // Purely a positioning/spacing wrapper — no background of its own, so the
  // card carousel stays visible scrolling behind/around the floating dock (its
  // own cards already reserve clearance for it via reservedNavigatorHeight).
  // paddingBottom is applied inline (see usage) as the ONE source of truth for
  // this dock's bottom clearance — Screen intentionally excludes the bottom
  // edge so the two can't stack into an oversized gap.
  navigatorDock: { position: 'absolute', right: 0, bottom: 0, left: 0, zIndex: 10, backgroundColor: 'transparent', paddingHorizontal: spacing.md, paddingTop: spacing.lg }, navigatorSurface: { borderRadius: 20, overflow: 'hidden', borderWidth: StyleSheet.hairlineWidth, shadowColor: '#000', shadowOpacity: 0.08, shadowRadius: 14, shadowOffset: { width: 0, height: 6 }, elevation: 4 }, navigator: { height: 44 }, navigatorContent: { position: 'relative', minHeight: 44, paddingHorizontal: spacing.md, alignItems: 'center', gap: spacing.xs }, navigatorIndicator: { position: 'absolute', bottom: 2, left: 0, width: 24, height: 2, borderRadius: 1 }, navigatorItem: { maxWidth: 176, minHeight: 44, justifyContent: 'center', paddingHorizontal: spacing.sm }, navigatorItemPressed: { opacity: 0.58 }, navigatorNameFrame: { maxWidth: 152, minHeight: 20, justifyContent: 'center' }, navigatorName: { maxWidth: 152, flexShrink: 1, fontSize: 14, lineHeight: 20 }, navigatorNameMeasure: { opacity: 0, fontWeight: '800' }, navigatorNameVisible: { position: 'absolute', right: 0, left: 0, fontWeight: '500', textAlign: 'center' }, navigatorNameSelected: { fontWeight: '800' }, navigatorPosition: { height: 16, fontSize: 10, lineHeight: 13, fontWeight: '600', textAlign: 'center', letterSpacing: 0.2 }, navigatorAddItem: { width: 30, height: 30, alignItems: 'center', justifyContent: 'center', borderRadius: 15, borderWidth: 1, borderStyle: 'dashed' },
  ...
  card: { overflow: 'hidden', borderRadius: 22, borderWidth: StyleSheet.hairlineWidth, shadowColor: '#000', shadowOpacity: 0.05, shadowRadius: 10, shadowOffset: { width: 0, height: 4 }, elevation: 2 }, cardHighlight: { zIndex: 1, borderRadius: 22, borderWidth: 2 }, cardMedia: { width: '100%', borderBottomWidth: StyleSheet.hairlineWidth },
  // Horizontal trimmed slightly, vertical cut substantially — vertical padding
  // (top+bottom, applied twice per card) is what was making every card read as
  // oversized, far more than the horizontal margins. `gap` is between cardTop,
  // the metadata row, the divider, and the controls row — one value, since RN's
  // flex `gap` applies uniformly between direct children, so tightening it here
  // is what closes up the space around the divider on both sides at once.
  cardBody: { paddingHorizontal: 16, paddingVertical: 12, gap: spacing.xs },
  cardPressed: { opacity: 0.78 }, cardDragging: { zIndex: 30, elevation: 12, shadowColor: '#000', shadowOpacity: 0.16, shadowRadius: 14, shadowOffset: { width: 0, height: 7 } },
  ...
  dragHandle: { width: 44, height: 44, alignItems: 'center', justifyContent: 'center', marginTop: -spacing.xs, marginRight: -spacing.xs }, dotGrid: { width: 12, height: 16, flexDirection: 'row', flexWrap: 'wrap', alignContent: 'center', justifyContent: 'space-between', gap: 2 }, dot: { width: 3, height: 3, borderRadius: 1.5 }, preview: { marginTop: spacing.xxs, fontSize: 14, lineHeight: 20 }, meta: { minHeight: 20, flexDirection: 'row', flexWrap: 'wrap', alignItems: 'center', gap: spacing.sm }, metaItem: { flexDirection: 'row', alignItems: 'center', gap: 4 }, metaText: { fontSize: 11, lineHeight: 15, fontWeight: '600' }, cardDivider: { height: StyleSheet.hairlineWidth },
  // Move/reorder: same "shrink the visible surface, keep the tap target"
  // approach as the drag handle — minHeight/height drop from 44/40 to 32, and
  // the Pressables below add vertical hitSlop to recover a ~44pt effective
  // touch target despite the smaller visible pill.
  cardControls: { minHeight: 32, flexDirection: 'row', alignItems: 'center', justifyContent: 'space-between', gap: spacing.sm }, move: { minHeight: 32, flexDirection: 'row', alignItems: 'center', justifyContent: 'center', gap: 4, paddingHorizontal: spacing.sm, borderRadius: 10 }, moveText: { fontSize: 12, fontWeight: '700' }, reorderGroup: { height: 32, flexDirection: 'row', alignItems: 'center', overflow: 'hidden', borderRadius: 10 }, reorderButton: { width: 36, height: 32, alignItems: 'center', justifyContent: 'center' }, reorderDivider: { width: StyleSheet.hairlineWidth, height: 16 }, reorderButtonDisabled: { opacity: 0.26 }, reorderButtonPressed: { opacity: 0.55 }, controlPressed: { opacity: 0.62 }, placeholder: { borderWidth: 1, borderStyle: 'dashed', borderRadius: 22 }, dragLayer: { position: 'absolute', zIndex: 50, elevation: 14 }, edgeGlow: { position: 'absolute', top: 0, bottom: 0, width: 28, zIndex: 40 }, edgeGlowLeft: { left: 0, borderTopRightRadius: 26, borderBottomRightRadius: 26 }, edgeGlowRight: { right: 0, borderTopLeftRadius: 26, borderBottomLeftRadius: 26 },
