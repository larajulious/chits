...
export function RenameBoardSheet({ visible, boardName, onClose, onSave }: Props) {
  const { tokens: theme } = useTheme();
  const inputRef = useRef<TextInput>(null);
  const wasVisible = useRef(false);
  const [name, setName] = useState(boardName);
  const [originalName, setOriginalName] = useState(boardName);
  ...
  const save = async () => {
    ...
    try {
      ...
      setError('Couldn’t rename this board. Try again.');
    } finally {
      setSaving(false);
    }
  };
  ...
  return (
    ...
    >
      ...
      >
        <Pressable accessibilityRole="button" accessibilityLabel="Cancel renaming board" style={StyleSheet.absoluteFill} onPress={close} />
        <SafeAreaView edges={['bottom']} style={[styles.safeArea, { backgroundColor: theme.surface }]}>
          <GlassSurface style={[styles.sheet, { borderColor: theme.borderSubtle }]}>
            ...
            <View style={styles.content}>
              <Text accessibilityRole="header" style={[styles.title, { color: theme.textPrimary }]}>Rename board</Text>
              <Text style={[styles.supportingCopy, { color: theme.textSecondary }]}>Give this board a clear name.</Text>
              ...
              <View style={styles.actions}>
                ...
                >
                  {saving ? <ActivityIndicator accessibilityLabel="Saving board name" size="small" color={theme.accentText} /> : <Text style={[styles.saveText, { color: theme.accentText }]}>Save</Text>}
                </Pressable>
              </View>
            </View>
          </GlassSurface>
